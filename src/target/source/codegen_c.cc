...
void CodeGenC::AddFunction(const PrimFunc& f) {
  ...
  if (find_U == true && (find_V | find_M) == false) {
   ...
   // if (k_pack4 == 0 && c == 0 && get_global_id(2) == 0) {
   //     float4 d00 = g00, d01 = g01, d02 = g02, d03 = g10, d10 = g11, d11 = g12, d12 = g20, d13 = g21,d14= g22;
   //     printf("%.1f,%.1f,%.1f,%.1f  %.1f,%.1f,%.1f,%.1f ,%.1f  \n",   d00.x, d01.x, d02.x, d03.x, d10.x, d11.x, d12.x, d13.x, d14.x);
   //     printf("%.1f,%.1f,%.1f,%.1f  %.1f,%.1f,%.1f,%.1f ,%.1f  \n",   d00.y, d01.y, d02.y, d03.y, d10.y, d11.y, d12.y, d13.y, d14.y);
   //     printf("%.1f,%.1f,%.1f,%.1f  %.1f,%.1f,%.1f,%.1f ,%.1f  \n",   d00.z, d01.z, d02.z, d03.z, d10.z, d11.z, d12.z, d13.z, d14.z);
   //     printf("%.1f,%.1f,%.1f,%.1f  %.1f,%.1f,%.1f,%.1f ,%.1f  \n\n", d00.w, d01.w, d02.w, d03.w, d10.w, d11.w, d12.w, d13.w, d14.w);
   // }
    float4 z00 = g00;
...
void CodeGenC::VisitExpr_(const AddNode* op, std::ostream& os) {  // NOLINT(*)
  if (const auto* bptr = op->a.as<BroadcastNode>()) {
    if (const auto* ptr = bptr->value.as<FloatImmNode>()) {
      if (fabs(ptr->value) <= 1e-15) {
        PrintExpr(op->b, os);
        return;
      }
    }
  }
  if (const auto* bptr = op->b.as<BroadcastNode>()) {
    if (const auto* ptr = bptr->value.as<FloatImmNode>()) {
      if (fabs(ptr->value) <= 1e-15) {
        PrintExpr(op->a, os);
        return;
...
void CodeGenC::VisitExpr_(const MulNode* op, std::ostream& os) {  // NOLINT(*)
  if (const auto* ptr = op->a.as<FloatImmNode>()) {
    bool c = ptr->value == 0;
    c = !c;
  }
  if (const auto* ptr = op->b.as<FloatImmNode>()) {
    bool c = ptr->value == 0;
    c = !c;
  }
  ...
  if (const auto* bptr = op->a.as<BroadcastNode>()) {
    if (const auto* ptr = bptr->value.as<FloatImmNode>()) {
      if (fabs(ptr->value) <= 1e-15) {
        PrintExpr(op->a, os);
        return;
      } else if (fabs(ptr->value - 1) <= 1e-15) {
        PrintExpr(op->b, os);
        return;
      }
    }
  ...
  if (const auto* ptr = op->a.as<IntImmNode>()) {
    bool c = ptr->value == 0;
    c = !c;
  }
  if (const auto* bptr = op->a.as<BroadcastNode>()) {
    if (const auto* ptr = bptr->value.as<IntImmNode>()) {
      if (ptr->value == 0) {
        PrintExpr(op->a, os);
        return;
      } else if (ptr->value == 1) {
  ...
  if (const auto* ptr = op->b.as<IntImmNode>()) {
    bool c = ptr->value == 0;
    c = !c;
  }
  if (const auto* bptr = op->b.as<BroadcastNode>()) {
