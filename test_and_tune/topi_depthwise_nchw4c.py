...
TRACKER_PORT=9090
@autotvm.template("conv2d_depthwise")
def conv2d_depthwise(N, H, W, CO, CI, KH, KW, stride, padding):
    ...
    kernel_pl = te.placeholder((1,K_P, KH, KW, 1, PACK4),
                               name='filter', dtype=ddtype)
    if "Mali" in device_key:
        conv_pl = topi.mali.depthwise_conv2d_NCHWc_mali_io(data_pl, kernel_pl, stride, padding, 1,
                                            'NCHWc', 'NCHWc',
                                            ddtype.replace('r', 'w'))
        s = topi.mali.schedule_depthwise_conv2d_NCHWc_io(conv_pl)
    else:
        conv_pl = topi.mali.depthwise_conv2d_NCHWc_io(data_pl, kernel_pl, stride, padding, 1,
                                            'NCHWc', 'NCHWc',
                                            ddtype.replace('r', 'w'))
        s = topi.mali.schedule_depthwise_conv2d_NCHWc_io(conv_pl)
...
def tune_and_evaluate(tuning_opt):
    ...
    tasks = []
    for net,shapes in conv_shape_tunning.conv_depthwise.items():
        for ith, shape in enumerate(shapes):
            N, CI, H, W, _, CO, KH, KW, strides, padding, dialte = shape
            if KH < 1 and KW < 1:
            ...
            task = autotvm.task.create(
                "conv2d_depthwise", args=(N, H, W, CO, CI, KH, KW, strides, padding), target=target,target_host=target_host
            )
            tasks.append(task)
    ...
    if 1:
        with tvm.target.Target("opencl"):
            s, arg_bufs = conv2d_depthwise(N, H, W, CO, CI, KH, KW, strides, padding)
