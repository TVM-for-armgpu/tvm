...
with open("kk.cl",'w') as fp:
    fp.write(func.imported_modules[0].get_source()) if len(func.imported_modules) > 0 else fp.write("source not imported")
...
c_tvm = tvm.nd.empty((1, CO, H, W), ctx=ctx, dtype=conv_pl.dtype)
#===================run tvm -===========
rlib = func
