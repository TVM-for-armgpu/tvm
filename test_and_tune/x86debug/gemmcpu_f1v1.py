...
wpo,wpii=s[B].split(wp_p4,factor=2*4)
...
#wpo,wpi=s[B].split(wpo,factor=block_factor)
kpo,kpi=s[B].split(kp,factor=block_factor*block_factor)
...
s[B].bind(wpo, block_z)
s[B].bind(kpo, block_x)
...
#s[B].bind(hpo, thread_y)
#s[B].bind(wpo, thread_x)
s[B].bind(kpi, thread_z)
s[B].reorder(wpo,hpo,kpo,hpi,kpi)
...
_, wpi = s[AL].split(wp_p4, factor=4)
...
wpio,wpii = s[B].split(wpii, factor=4)
