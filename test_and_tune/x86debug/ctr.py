...
rcoo,rcoi=s[B].split(rco,factor=4)
#kp, hp, wp_p4  = s[B].op.axis
#s[B].reorder(rci,rcoi,kp, hp, wp_p4)

BF = s.rfactor(B, rcoo,0)
a,b=s[BF].op.reduce_axis
c,d,e,f=s[BF].op.axis
s[BF].reorder(a,b,c,d,e,f)
...
# Split the workloads
kp, hp, wp_p4  = s[B].op.axis
#s[B].reorder(rci,wp_p4,hp,kp)
bwp,wp4=s[B].split(wp_p4,factor=4*2)
hpo,hpii=s[B].split(hp,factor=2)
kpo,kpi=s[B].split(kp,nparts=block_factor*16)
...
#wpo,wpi=s[B].split(wp,nparts=block_factor//2)
...
rc, = s[B].op.reduce_axis
s[B].reorder(hpo,kpo,bwp,hpii,kpi)
...
#s[B].reorder(wpo,hpo,kp,wpi,hpi,p4)
wp,p4 = s[B].split(wp4, factor=4)

#hpiifuse_wp=s[B].fuse(hpii,wp)
#-#s[B].reorder(hpii,wp,p4)
#-#whp = s[B].fuse(wp,hpii)


# Attach computation to iteration variables
#s[AA].compute_at(s[BL], rco)
#s[WW].compute_at(s[BL], rco)
rcin, kp, hp, wp_p4 = s[BF].op.axis
rcout = s[BF].op.reduce_axis[0]
s[BF].reorder(rcout,rcin,kp, hp, wp_p4)
s[BF].vectorize(rcin)
s[B].set_store_predicate(thread_x.var.equal(0))
s[AL].compute_at(s[B], s[B].op.reduce_axis[0])
s[WL].compute_at(s[B], s[B].op.reduce_axis[0])
...
#s[B].vectorize(wp)  # vectorize memory load
#s[B].unroll(wpio)
#s[B].unroll(hpii)
