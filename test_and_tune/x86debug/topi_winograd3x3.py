...
def conv2d_no_batching(N, H, W, CO, CI, KH, KW, stride, padding):
    ...
    conv_pl = topi.mali.conv2d_nchw_winograd_nchwc_conv3x3_io(data_pl, kernel_pl, 1, 1,
...
log_file = os.path.dirname(__file__)+"\\%s.%s.log" % (device_key, network)
dtype = "float32"
...
def tune_and_evaluate(tuning_opt):
    ...
    with autotvm.apply_history_best(log_file) as dispatch_context:
        ...
        c_tvm = tvm.nd.empty(
            arg_bufs[2].shape, ctx=ctx, dtype=arg_bufs[2].dtype)
