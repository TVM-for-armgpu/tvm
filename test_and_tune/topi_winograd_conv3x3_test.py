...
def iohw_iohw14(mat):
    CI_, CO_, kh, kw = mat.shape
    w_np_tvm = np.ndarray(0)
    for i in range(0, CO_, 4):
        b = mat[:, i:i + 4, :, :].transpose(0, 2, 3, 1)
...
def nchw_nchwc4(a_np):
    in_channel = a_np.shape[1]
...
def nchw_nc16ws4(a_np):
    N_, CI_, H_, W_ = a_np.shape
    data_only_tr = np.zeros((N_, CI_, 16, (H_ // 4 * (W_ // 4))))
    for c in range(N_):
        for k in range(CI_):
            inum = 0
            for i in range(0, H_ // 4):
                for j in range(0, W_ // 4):
                    data_only_tr[c, k, :,
                                 inum] = a_np[c, k, i * 4:i * 4 + 4,
                                              j * 4:j * 4 + 4].reshape(16)
                    inum = inum + 1
    data_only_tr = nchw_nchwc4(data_only_tr)
    return data_only_tr


def evaluate_shape_on_device(tasks, tuning_opt, shape):
    # run tuning tasks
    #print("Tuning...")
    print(tasks.args)
    ...
    with autotvm.apply_history_best(log_file) as dispatch_context:
        ...
        with tvm.target.Target("opencl"):
            ...
            with open('dd.cl', 'w') as fp:
                print(func.imported_modules[0].get_source(),
                      file=fp) if len(func.imported_modules) > 0 else print(
                          "source not imported", file=fp)
        # export library
        tmp = tempdir()
        if use_android:
        ...
        #===============tvm data format
        nchw_shape = (a_s[0], a_s[1] * a_s[-1], a_s[2], a_s[3])
        iohw_shape = (w_s[0], w_s[1] * w_s[-1], w_s[2], w_s[3])
        a_np = np.arange(0, 0.001 * np.prod(a_s), 0.001).reshape(nchw_shape)
        w_np = np.arange(0, 0.001 * np.prod(w_s), 0.001).reshape(iohw_shape)

        a_np_tvm = nchw_nchwc4(a_np)
        w_np_tvm = iohw_iohw14(w_np)

        a_tvm = tvm.nd.array(a_np_tvm, ctx=ctx, dtype=arg_bufs[0].dtype)
        w_tvm = tvm.nd.array(w_np_tvm, ctx=ctx, dtype=arg_bufs[1].dtype)
        c_tvm = tvm.nd.empty(arg_bufs[2].shape,
                             ctx=ctx,
                             dtype=arg_bufs[2].dtype)
        ####numpy calculate the answer over
        w_np = w_np.transpose(1, 0, 2, 3)

        time_f = rlib.time_evaluator(rlib.entry_name, ctx, number=15)
        ...
        c_tvm_o = c_tvm.asnumpy()
        #print("verify...")
        #c_np = conv2d_nchw_python(a_np, w_np, strides, padding)
        #c_np_verify = nchw_nchwc4(c_np)
        #tvm.testing.assert_allclose(c_np_verify, c_tvm_o, rtol=1e-2)
        ...
        ftimer = rlib.time_evaluator(rlib.entry_name, ctx, number=5, repeat=5)
        a = ftimer(a_tvm, w_tvm, c_tvm).results
        prof_res = np.array(a) * 1000  # convert to millisecond
        print("Mean inference time (std dev): %.2f ms (%.2f ms) %f GFLOPS" %
              (np.mean(prof_res), np.std(prof_res),
               W * H * CI * CO * 9 * 2 / np.mean(prof_res) / 1e6))
        return
...
def tune_and_evaluate(tuning_opt):
    ...
    tasks = []
    for net, shapes in conv_shape_tunning.conv3x3shape_d.items():
        for ith, shape in enumerate(shapes):
            ...
            evaluate_shape_on_device(tasks, tuning_opt, shape)
...
if __name__ == '__main__':
    ...
    #log_file = f'best_{device_key}.wino3x3.log'
    tuning_option = {
        "log_filename":
        log_file,
        "tuner":
