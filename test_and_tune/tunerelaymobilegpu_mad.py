...
def conv2d_nchw(Input, Filter, stride, padding, dilation, out_dtype=None):
    ...
    out_height = topi.nn.simplify((in_height - dilated_kernel_h + pad_top + pad_down) // stride_h + 1)
    out_width = topi.nn.simplify((in_width - dilated_kernel_w + pad_left + pad_right) // stride_w + 1)
    # compute graph
    ...
    return te.compute(
        ...
        lambda nn, ff, yy, xx: extern_op.mysum(
            extern_op.mymul(temp[nn, rc, yy * stride_h + ry * dilation_h, xx * stride_w + rx * dilation_w].astype(
                out_dtype
            )
...
def conv2d_no_batching(N, H, W, CO, CI, KH, KW, stride, padding):
    ...
    conv = conv2d_nchw(data, kernel, stride, padding, dilation=0, out_dtype="climgfloatw32")
