...
def conv2d_no_batching(N, H, W, CO, CI, KH, KW, stride, padding):
    ...
    data_pl = te.placeholder((1, CI, H, W), name='data', dtype=ddtype)
    kernel_pl = te.placeholder((CO, CI, KH, KW), name='filter', dtype=ddtype)
    if "Mali" in device_key:
        conv_pl = topi.mali.conv2d_nchw_spatial_pack(data_pl, kernel_pl,
                                                     stride, padding, 1,
                                                     ddtype.replace('r', 'w'))
    else:
        conv_pl = topi.mali.conv2d_nchw_spatial_pack(data_pl, kernel_pl,
                                                     stride, padding, 1,
                                                     ddtype.replace('r', 'w'))
    s = topi.mali.schedule_conv2d_nchw_spatial_pack([conv_pl])
    a_data, a_kernel = conv_pl.op.input_tensors[0].op.input_tensors
    if not autotvm.GLOBAL_SCOPE.in_tuning:
        a_kernel = kernel_pl
    #print(tvm.lower(s, [data_pl,kernel_pl,conv_pl], simple_mode=True))
    #exit(0)

    return s, [data_pl, a_kernel, conv_pl]
...
def tune_tasks(
    ...
    for i, tsk in enumerate(reversed(tasks)):
        if in_check_point(tsk.args):
            print(f"skip finished {i+1}th task",tsk.args)
            continue
...
def tune_and_evaluate(tuning_opt):
    ...
    if 1:
        ...
        tvm.testing.assert_allclose(c_np, c_tvm_o, rtol=1e-1)
        # evaluate
        print("answer check passed. Evaluate inference time cost... ")
        ftimer = rlib.time_evaluator(rlib.entry_name, ctx, number=3, repeat=3)
