...
# (1). :ref:`opt-conv-pu`
# (2). `Optimizing DepthwiseConv on NVIDIA PU <https://tvm.apache.org/2017/08/22/Optimize-Deep-Learning-PU-Operators-with-TVM-A-Depthwise-Convolution-Example>`_
#
...
#
# If you are familiar with writing opencl schedule, you can find the following
...
arch = "arm64"
_hosttarget_host = "llvm -mtriple=%s-linux-android" % arch
...
N, H, W, CO, CI, KH, KW, strides, padding = 1, 38, 38, 512, 256, 1, 1, (1, 1), (1, 1)
task = autotvm.task.create(
    "tutorial/conv2d_no_batching", args=(N, H, W, CO, CI, KH, KW, strides, padding), target=target,target_host=target_host
)
print(task.config_space)

# Use local pu, measure 10 times for every config to reduce variance
...
with autotvm.apply_history_best("conv2d.log"):
    with tvm.target.Target("opencl"):
        ...
        func = tvm.build(s, arg_bufs, target_host=target_host)
...
filename = path_dso_cl
print("Upload...")
remote = autotvm.measure.request_remote(device_key, tracker_host,tracker_port , timeout=10000)
